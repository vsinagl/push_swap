...
int	get_index(int from_head, t_stack *to, int head)
{
	...
	if ((int)(to->len) == 0)
		index = 1;
	while (index < (int)(to->len) &&
			(from_head > get_stack_value(to, index - head)))
		index++;
...
t_solver	*create_solver(t_stack *from, t_stack *to,
		int head_from, int head_to)
{
	t_solver	*solver;
	int			i;
	int			b_score;
	...
	{
		solver->matrix[0][i] = rotation_distance((int)(from->len),
				head_from, i, (int)(from->len));
